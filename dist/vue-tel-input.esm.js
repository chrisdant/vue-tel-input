...
var script = {
  ...
  props: {
    ...
    ariaDescribedby: {
      type: String,
      default: function _default() {
        return getDefault('ariaDescribedby');
      }
    },
    ariaLabel: {
      type: String,
      default: function _default() {
        return getDefault('ariaLabel');
      }
    },
    ariaLabelledby: {
      type: String,
      default: function _default() {
        return getDefault('ariaLabelledby');
      }
    },
    ariaInvalid: {
      type: Boolean,
      default: function _default() {
        return getDefault('ariaInvalid');
      }
    },
    autocomplete: {
      type: String,
  ...
  watch: {
    ...
    phone: function phone(newValue, oldValue) {
      ...
      if (this.cursorPosition < oldValue.length && // Selecting in dropdown shouldn't move focus to input for accessibility reasons
    ...
    keyboardNav: function keyboardNav(e) {
      ...
      if (e.keyCode === 40) {
        ...
        if (this.open) {
          ...
          if (_selEle2.offsetTop < this.$refs.list.scrollTop) {
            this.$refs.list.scrollTop = _selEle2.offsetTop;
          }
        }
        ...
        e.preventDefault();
        this.selectedIndex = this.sortedCountries.length - 1;
        var _selEle3 = this.$refs.list.children[this.selectedIndex];

        if (_selEle3.offsetTop + _selEle3.clientHeight > this.$refs.list.scrollTop + this.$refs.list.clientHeight) {
          this.$refs.list.scrollTop = _selEle3.offsetTop - this.$refs.list.clientHeight + _selEle3.clientHeight;
        }
      } else {
        ...
        if (typedCountryI >= 0) {
          ...
          var _selEle4 = this.$refs.list.children[this.selectedIndex];
          var needToScrollTop = _selEle4.offsetTop < this.$refs.list.scrollTop;
          var needToScrollBottom = _selEle4.offsetTop + _selEle4.clientHeight > this.$refs.list.scrollTop + this.$refs.list.clientHeight;

          if (needToScrollTop || needToScrollBottom) {
            this.$refs.list.scrollTop = _selEle4.offsetTop - this.$refs.list.clientHeight / 2;
          }
        }
      }
    },
    // Catch keyup events for keys used by this component so they don't cause problems for parent elements.
    catchKeyUpEvents: function catchKeyUpEvents(e) {
      var keyCodes = [40, 38, 13, 33, 34, 27];
...
var __vue_render__ = function __vue_render__() {
  ...
  return _c('div', {
    ...
    attrs: {
      ...
      "role": "button",
      "aria-label": "Select a country: " + _vm.activeCountry.name,
      "aria-haspopup": "listbox",
      "aria-expanded": _vm.open
    },
    on: {
      "keyup": _vm.catchKeyUpEvents,
    ...
    attrs: {
      ...
      "aria-activedescendant": _vm.selectedIndex !== null && _vm.open ? _vm.inputId + "-dropdown-item-" + _vm.selectedIndex : null
    }
    ...
    return _c('li', {
      ...
      attrs: {
        "id": _vm.inputId + "-dropdown-item-" + index,
        "role": "option",
        "aria-selected": index === _vm.selectedIndex && _vm.open
      },
      on: {
    ...
    attrs: {
      ...
      "aria-labelledby": _vm.ariaLabelledby ? _vm.ariaLabelledby : null,
      "aria-describedby": _vm.ariaDescribedby ? _vm.ariaDescribedby : null,
      "aria-invalid": _vm.ariaInvalid
    },
    domProps: {
      "value": _vm.phone
    },
